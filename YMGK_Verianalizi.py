...
airQuality= pd.read_excel("Seferihisar.xlsx")
...
def AverageDifference():
    ...
    print("PM10 Hafta içi : {0} , PM10 HaftaSonu : {1}".format(str(weekDayPM10), str(weekEndPM10)))

    weekDaySO2 = Average(dictWeekDayAverage["SO2"])
    weekEndSO2 = Average(dictWeekEndAverage["SO2"])
    print("SO2 Hafta içi : {0} , SO2 HaftaSonu : {1}".format(str(weekDaySO2), str(weekEndSO2)))

    weekDayCO = Average(dictWeekDayAverage["CO"])
    weekEndCO = Average(dictWeekEndAverage["CO"])
    print("CO Hafta içi : {0} , CO HaftaSonu : {1}".format(str(weekDayCO), str(weekEndCO)))

    weekDayNO2 = Average(dictWeekDayAverage["NO2"])
    weekEndNO2 = Average(dictWeekEndAverage["NO2"])
    print("NO2 Hafta içi : {0} , NO2 HaftaSonu : {1}".format(str(weekDayNO2), str(weekEndNO2)))

    weekDayNOX = Average(dictWeekDayAverage["NOX"])
    weekEndNOX = Average(dictWeekEndAverage["NOX"])
    print("NOX Hafta içi : {0} , NOX HaftaSonu : {1}".format(str(weekDayNOX), str(weekEndNOX)))

    weekDayNO = Average(dictWeekDayAverage["NO"])
    weekEndNO = Average(dictWeekEndAverage["NO"])
    print("NO Hafta içi : {0} , NO HaftaSonu : {1}".format(str(weekDayNO), str(weekEndNO)))

    weekDayO3 = Average(dictWeekDayAverage["O3"])
    weekEndO3 = Average(dictWeekEndAverage["O3"])
    print("O3 Hafta içi : {0} , O3 HaftaSonu : {1}".format(str(weekDayO3), str(weekEndO3)))

    weekDayPM25 = Average(dictWeekDayAverage["PM2.5"])
    weekEndPM25 = Average(dictWeekEndAverage["PM2.5"])
    print("PM25 Hafta içi : {0} , PM25 HaftaSonu : {1}".format(str(weekDayPM25), str(weekEndPM25)))

    weekDayAQI = Average(dictWeekDayAverage["AQI"])
    weekEndAQI = Average(dictWeekEndAverage["AQI"])
    print("AQI Hafta içi : {0} , AQI HaftaSonu : {1}".format(str(weekDayAQI), str(weekEndAQI)))
